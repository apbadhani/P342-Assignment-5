...
# Runge-Kutta method
def rk4(f1,f2,h,y1,y2,xn,x):
    n=int((xn-x)/h)
    dx=h
    for i in range (n):
        k11=dx*f1(y1,y2,x)
        k21=dx*f2(y1,y2,x)
        k12=dx*f1(y1+k11/2,y2+k21/2,x+dx/2)
        k22=dx*f2(y1+k11/2,y2+k21/2,x+dx/2)
        k13=dx*f1(y1+k12/2,y1+k22/2,x+dx/2)
        k23=dx*f2(y1+k12/2,y2+k22/2,x+dx/2)
        k14=dx*f1(y1+k13,y2+k23,x+dx)
        k24=dx*f2(y1+k13,y2+k23,x+dx)
        y1n=y1+(k11+2*k12+2*k13+k14)/6
        y2n=y2+(k21+2*k22+2*k23+k24)/6
        print('%.5f\t%.5f\t' % (x, y1))
        y1=y1n
        x=x+dx
        y2=y2n
